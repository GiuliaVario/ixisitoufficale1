  ...
  return (
    <section className="bg-[var(--brand-bg)] text-[var(--brand-fg)] pt-12 md:pt-16 pb-16 md:pb-24">
      <Container>
        {/* Titolo */}
        <h2
          className="impact font-title font-black uppercase text-right leading-[0.9] tracking-[-0.01em] mb-12 md:mb-16"
          style={{
            fontSize: 'clamp(3rem, 8vw, 6.5rem)',
            color: 'var(--brand-accent-1)', // verde lime
          }}
        >
          PROCESSO
        </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-10">
          {STEPS.map((s) => (
            <div
              key={s.n}
              className="border border-white/90 p-6 md:p-7 rounded-2xl bg-transparent transition-transform duration-200 hover:scale-[1.03]"
            >
              <div className="impact text-[var(--brand-accent-1)] text-[2.4rem] md:text-[3rem] leading-none">
                {s.n}
              </div>
              <h3 className="font-title font-extrabold uppercase mt-3 text-[1.2rem] md:text-[1.3rem] text-white">
                {s.title}
              </h3>
              <p className="mt-3 text-white/90 leading-relaxed text-[1.05rem]">
                {s.text}
              </p>
            </div>
          ))}
